...
public class TellerPagingRepository<DS: PagingRepositoryDataSource>: TellerRepository<DS> {
    ...
    public var areMorePagesAvailable: Bool {
        return nextPageRequirements != nil
    }
    ...
    public func goToNextPage() {
        guard let nextPageRequirements = self.nextPageRequirements else {
     ...
     * 1. Querying the cache after opening the screen of the app for the first time. When we are paging the cache, we are only displaying the first page of the cache and we delete all of the rest. Because we deleted the old data, we need to always perform a refresh
